document.addEventListener('DOMContentLoaded', function () {
    ...
    /**
     * Populates and shows the metrics panel with a global overview.
     */
    ...
    function loadCity(cityId) {
        ...
        // --- Update and Show City-Specific Metrics Panel ---
        ...
        const lineColorExpression = ['match', ['get', 'cluster'],
            0, clusterInfo.A.color,
            1, clusterInfo.B.color,
            3, clusterInfo.C.color,
            2, clusterInfo.D.color,
        ...
        map.on('mousemove', 'segments-layer', (e) => { if ('ontouchstart' in window) return; map.getCanvas().style.cursor = 'pointer'; if (e.features.length > 0) segmentPopup.setLngLat(e.lngLat).setHTML(createPopupContent(e.features[0].properties)).addTo(map); });
        map.on('mouseleave', 'segments-layer', () => { if ('ontouchstart' in window) return; map.getCanvas().style.cursor = ''; segmentPopup.remove(); });
        map.on('click', 'segments-layer', (e) => { if (e.features.length > 0) segmentPopup.setLngLat(e.lngLat).setHTML(createPopupContent(e.features[0].properties)).addTo(map); });
    }
    ...
    map.on('load', function() {
        ...
        // Fetch data files and populate UI
        Promise.all([
        ...
        metricsToggle.addEventListener('click', () => {
            if (metricsPanel.classList.contains('visible') && document.getElementById('metrics-panel-title').textContent !== 'Global Overview') {
